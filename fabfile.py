...
env.directory = {
    'production': '/home/%s/projects/cetaganda' % SSH_USER,
}
...
def init():
    with settings(user='root'):
        ...
        if not exists('/var/log/projects/cetaganda'):
            run('mkdir -p /var/log/projects/cetaganda')
            run('chmod 777 /var/log/projects/cetaganda')
            run('touch /var/log/projects/cetaganda/restart.log')
            run('chown -R %(user)s:%(user)s /var/log/projects/cetaganda' % {'user': SSH_USER})
        ...
        if not exists('/etc/nginx/sites-available/90-cetaganda.conf'):
            run('touch /etc/nginx/sites-available/90-cetaganda.conf')
            run('chown %s /etc/nginx/sites-available/90-cetaganda.conf' % SSH_USER)
        if not exists('/etc/nginx/sites-enabled/90-cetaganda.conf'):
            run('ln -s /etc/nginx/sites-available/90-cetaganda.conf /etc/nginx/sites-enabled/90-cetaganda.conf', shell=False)

        if not exists('/etc/init/cetaganda.conf'):
...
def nginx():
    run('cp %s/tools/nginx/90-cetaganda.conf /etc/nginx/sites-available/90-cetaganda.conf' % env.directory[env.env_type], shell=False)


def upstart():
    run('cp %s/tools/upstart/cetaganda.conf /etc/init/cetaganda.conf' % env.directory[env.env_type], shell=False)


def dump():
    with cd(env.directory[env.env_type]):
        tmp_filename = run("date +/tmp/cetaganda_backup_%Y%m%d_%H%M.sql.gz")
